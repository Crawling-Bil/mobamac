...
extension AcceptsVersionMessages {
    ...
    private func validateVersion(_ version: Substring.UTF8View) throws -> Bool {
        if version.count > 7, version.starts(with: "SSH-".utf8) {
            ...
            if version[start ..< end].elementsEqual(Substring("2.0").utf8) {
                return true
            }
            // MobaMac patch. RFC 4253 section 5.1: a server that speaks SSH-2
            // but also still accepts SSH-1 clients identifies itself as
            // "1.99", and "Clients using protocol 2.0 MUST be able to identify
            // this as identical to '2.0'". Cisco IOS and PAN-OS both do this
            // by default. `count > 7` above already guarantees the four bytes
            // at 4..<8 exist. The banner itself is passed through untouched
            // (it is hashed into the key exchange as V_S), only the check
            // changes.
            let end199 = version.index(start, offsetBy: 4)
            if version[start ..< end199].elementsEqual(Substring("1.99").utf8) {
                return true
            }
